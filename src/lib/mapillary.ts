...
function seededNumber(seedKey: string): number {
  let h = 0;
  for (let i = 0; i < seedKey.length; i++) {
    h = (Math.imul(31, h) + seedKey.charCodeAt(i)) | 0;
  }
  return Math.abs(h);
}
...
function seededShuffle<T>(arr: T[], seedKey: string): T[] {
  const result = [...arr];
  const rng = seededRandom(seededNumber(seedKey));
...
export async function fetchMapillaryCity(
  ...
  const citySeedKey = seedKey ?? `${lat},${lng}`;
  ...
  for (let attempt = 0; attempt < maxRetries; attempt++) {
    try {
      ...
      const shuffled = seededShuffle(allImages, `${citySeedKey}-attempt-${attempt}`).slice(0, 5);
