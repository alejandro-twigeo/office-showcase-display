      ...
      <div className="h-full w-full p-2 md:p-[clamp(16px,1.6vw,28px)]">
        <div className="h-full w-full grid grid-rows-[auto_1fr] gap-[clamp(12px,1.2vw,20px)]">
          ...
          <header className="flex items-center gap-2 md:gap-[clamp(12px,1.2vw,24px)]">
            ...
            {/* centered rotating message — hidden on mobile unless desktop mode is forced */}
            ...
            <div className="flex items-center gap-2 md:gap-[clamp(8px,0.7vw,14px)] ml-auto">
              {!forceDesktop && (
                <button
                  ...
                  className="md:hidden shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-full border-2 border-border text-muted-foreground hover:bg-muted transition-colors text-sm font-medium"
          ...
          <div className={`min-h-0 ${forceDesktop ? "overflow-hidden" : "overflow-y-auto md:overflow-hidden"}`}>
            <div
              className={`${
                forceDesktop
                  ? "h-full grid grid-rows-[1fr_1fr]"
                  : "h-auto md:h-full flex flex-col md:grid md:grid-rows-[1fr_1fr]"
              } gap-[clamp(12px,1.2vw,20px)] md:min-h-0`}
            >
              <div
                className={`${
                  forceDesktop ? "min-h-0 grid grid-cols-3" : "min-h-0 flex flex-col md:grid md:grid-cols-3"
                } gap-[clamp(12px,1.2vw,20px)]`}
              >
                <div className="min-h-0 md:h-full overflow-hidden rounded-xl">
                  <YouTubeDisplay />
                </div>
              ...
              <div className="min-h-0 overflow-hidden rounded-xl">
                <StreetViewDisplay forceDesktop={forceDesktop} />
              </div>
            </div>
          </div>
        </div>
